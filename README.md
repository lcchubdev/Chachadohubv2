...
-- Fundo vermelho gradiente animado
...
Gradient.Color = ColorSequence.new{
	ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 0, 0)),     -- Vermelho vibrante
	ColorSequenceKeypoint.new(0.5, Color3.fromRGB(120, 0, 0)),   -- Vermelho escuro
...
BarBG.BackgroundColor3 = Color3.fromRGB(80, 0, 0)
BarBG.Parent = Frame
...
Bar.BackgroundColor3 = Color3.fromRGB(255, 50, 50)  -- Vermelho neon
Bar.Parent = BarBG
...
-- Foto do usuÃƒÂ¡rio centralizada
...
task.spawn(function()
	...
	while ScreenGui.Parent do
		...
		Nick.TextColor3 = Color3.fromRGB(
			math.clamp(255, 200, 255),
			math.clamp(100 + 50 * math.sin(t*2), 50, 150),
			math.clamp(100 + 50 * math.sin(t*2), 50, 150)
		)
		task.wait(0.03)
	end
...
task.spawn(function()
	...
	while ScreenGui.Parent do
		...
		local r = 255
		local g = 50 + 50 * math.sin(t*1.5)
		local b = 50 + 50 * math.cos(t*1.5)
		Label.TextColor3 = Color3.fromRGB(
			math.clamp(r, 200, 255),
			math.clamp(g, 0, 100),
			math.clamp(b, 0, 100)
		)
...
GameLabel.TextColor3 = Color3.fromRGB(255, 150, 150)
...
for i=1,30 do
	...
	spark.BackgroundColor3 = Color3.fromRGB(255, 50, 50)
...
local redzlibContent = game:HttpGet("https://pastefy.app/IrFGYT9H/raw", true)
...
local Window = redzlib:MakeWindow({
    Title = Translator:traduzir("chachado hub"),
    SubTitle = "By:ixi362 and chachado",
...
local args = {
    [1] = "RolePlayName",
    [2] = "UsuÃƒÂ¡rio do Hub: [Chachado hub☕]"
}
...
local Tabs = {
    M = Window:MakeTab({Translator:traduzir("informaÃƒÂ§ÃƒÂµes"), "Info"}),
    H = Window:MakeTab({Translator:traduzir("InÃƒÂ­cio"), "coffee"}),
    PR = Window:MakeTab({Translator:traduzir("ProteÃƒÂ§ÃƒÂ£o"), "shield"}),
    ...
    CFG = Window:MakeTab({Translator:traduzir("ConfiguraÃƒÂ§ÃƒÂµes"), "Settings"})
}

Tabs.M:AddDiscordInvite({
    Name = Translator:traduzir("Chachado hub"),
...
do
Tabs.M:AddSection({Translator:traduzir("ÃƒÂnuncios <announcement>")})
Tabs.M:AddButton({Translator:traduzir("Aviso Externo"), function()
Window:Dialog({
    Title = "Erros",
    Text = Translator:traduzir("o hub aí deve créditos ao HIRO blz?."),
    Options = {
...
Window:Dialog({
    Title = "Update",
    Text = Translator:traduzir(A galerinha o hub pode ter alguns bugs de pois resolvo kksks."),
    Options = {
        {"Link Discord", function()
            copyDiscordLink()
        end},
...
local Section = Tabs.M:AddSection({Translator:traduzir("ResoluÃƒÂ§ÃƒÂµes Da Tela")})

        Tabs.M:AddButton({Translator:traduzir("ResoluÃƒÂ§ÃƒÂ£o Normal"), 
          function()
          getgenv().Resolution = {
            [".gg/scripters"] = 1.0
        }
...
getgenv().gg_scripters = "Aori0001"
        ...
        Tabs.M:AddButton({Translator:traduzir("ResoluÃƒÂ§ÃƒÂ£o 0.70"),
          function()
            getgenv().Resolution = {
        [".gg/scripters"] = 0.70
    }
...
getgenv().gg_scripters = "Aori0001"
        ...
        Tabs.M:AddButton({Translator:traduzir("ResoluÃƒÂ§ÃƒÂ£o 0.80"), 
          function()
                    getgenv().Resolution = {
            [".gg/scripters"] = 0.80
        }
...
Tabs.H:AddSection({Translator:traduzir("Status AtualizaÃƒÂ§ÃƒÂ£o")})
    ...
    while true do
        ...
        if scout then
            ...
            if hrpScout and hrpLocal then
                ...
                texto = texto .. string.format("estÃƒÂ¡ no servidor | DistÃƒÂ¢ncia <%d>", distancia)
            else
                texto = texto .. "estÃƒÂ¡ no servidor | DistÃƒÂ¢ncia <???>"
            end
        else
            texto = texto .. "nÃƒÂ£o estÃƒÂ¡ no servidor"
        end
...
Toggle:Callback(function(state)
        ...
        if not (Player and Backpack and Mouse and Character and Humanoid and RootPart and Clone) then
            warn("Erro: alguma instÃƒÆ’Ã‚Â¢ncia necessÃƒÆ’Ã‚Â¡ria nÃƒÆ’Ã‚Â£o foi encontrada.")
            return
        end
...
Tabs.H:AddToggle({
    Name = Translator:traduzir("Cabelo RGB"),
    Description = Translator:traduzir("Mudar a cor de cabelo automÃƒÂ¡tico"),
    Default = false,
    Callback = function(value)
        isHairActive = value
    end
...
Tabs.H:AddToggle({
    ...
    Description = Translator:traduzir("Mudar a cor do nome automÃƒÂ¡tico"),
    Default = false,
    Callback = function(value)
        isNameActive = value
    end
...
Tabs.H:AddToggle({
    Name = Translator:traduzir("Bio RGB"),
    Description = Translator:traduzir("Mudar a cor da bio automÃƒÂ¡tico"),
    Default = false,
    Callback = function(value)
        isBioActive = value
    end
...
Tabs.H:AddToggle({
    Name = Translator:traduzir("Rosas RGB"),
...
Tabs.H:AddToggle({
    Name = Translator:traduzir("Personagem RGB"),
...
Tabs.H:AddSection({Translator:traduzir("RepetiÃƒÂ§ÃƒÂ£o De Nomes")})
...
Tabs.H:AddTextBox({
    ...
    Description = Translator:traduzir("Primeiro Nome Da RepetiÃƒÂ§ÃƒÂ£o!"),
    Default = "",
...
Tabs.H:AddTextBox({
    Title = Translator:traduzir("Nome"),
    Description = Translator:traduzir("Segundo Nome Da RepetiÃƒÂ§ÃƒÂ£o!"),
    Default = "",
    PlaceholderText = "Nick Aqui",
    ClearText = false,
    Callback = function(N)
...
Tabs.H:AddTextBox({
    Title = Translator:traduzir("Nome"),
    Description = Translator:traduzir("Terceiro Nome Da RepetiÃƒÂ§ÃƒÂ£o!"),
    Default = "",
    PlaceholderText = "Nick Aqui",
    ClearText = false,
    Callback = function(N)
...
local Toggle = Tabs.H:AddToggle({
    Name = Translator:traduzir("NOME RGB"),
    Description = Translator:traduzir("mudar a cor dos nomes automÃƒÂ¡tico"),
    Default = false
})
...
-----------------///////TABS - PR///////-----------------
Tabs.PR:AddSection({Translator:traduzir("ProteÃƒÂ§ÃƒÂµes Local & Anti Lags")})
local Players = game:GetService("Players")
local Workspace = game:GetService("Workspace")
...
local AntiChatSpy_Toggle = Tabs.PR:AddToggle({
    ...
    Description = Translator:traduzir("Impede que os corno vejam seu chat<Creator>"),
    Default = false
})
...
local Toggle3 = Tabs.PR:AddToggle({Name = Translator:traduzir("Anti Script"), Description = Translator:traduzir("ProteÃƒÂ§ÃƒÂ£o contra colisÃƒÂµes de veÃƒÂ­culos/objetos"), Default = false})
...
local Button = Tabs.PR:AddButton({Translator:traduzir("Anti Lag"), function()
  ...
  local Dialog = Window:Dialog({
    Title = Translator:traduzir("Oque ÃƒÂ© isso?"),
...
Tabs.Client:AddToggle({
    Name = Translator:traduzir("Mensagem AutomÃƒÂ¡tica"),
    Default = false,
    Callback = function(value)
...
Tabs.Client:AddButton({
    ...
    Callback = function()
        if tcs.ChatVersion == Enum.ChatVersion.TextChatService then 
            local clearText = "hi" .. string.rep("\r", 200) .. "Server: Phantom Client"
...
local predefinedMessages = {
    {"ixi362 te encontrou!", "ixi362 te encontrou!"},
    {"Vou Trava Geral", "Vou Trava Geral"},
    {"E o Bonde do chachado", "E o Bonde do chachado"},
...
local espToggles = {
    {name = "Linhas <Esp>", key = "showLine", desc = "Mostra linhas da posiÃƒÂ§ÃƒÂ£o dos jogadores"},
    ...
    {name = "DistÃƒÂ¢ncia <Esp>", key = "showDistance", desc = "Mostra as distÃƒÂ¢ncias dos jogadores"}
}
...
Tabs.Client:AddButton({
    ...
    function()
        Window:Dialog({
            Title = Translator:traduzir("FunÃƒÂ§ÃƒÂµes Do jatos"),
            Text = Translator:traduzir("Ã¢â‚¬Â¢ Para funcionar precisa de GamePass <Premium> ÃƒÂ© versÃƒÂ£o Beta. Pode conter erros. Se demorar, recomendo ativar e desativar!"),
            Options = {
                {"Fechar", function() end}
            }
        })
...
Tabs.Client:AddButton({
    ...
    function()
        Window:Dialog({
            ...
            Text = Translator:traduzir("O spam comeÃƒÂ§a em atÃƒÂ© 20 segundos. NÃƒÂ£o ÃƒÂ© visual. ApÃƒÂ³s usar, nÃƒÂ£o toque em nada, apenas no script."),
            Options = {
                {"ComeÃƒÂ§ar Spam", function()
...
local playerDropdown = Tabs.Client:AddDropdown({
    Name = Translator:traduzir("Selecionar Jogadore"),
...
Tabs.Client:AddToggle({
    Name = Translator:traduzir("
ver mente do loko"),
    Description = Translator:traduzir("Mesmo de longe vocÃƒÂª consegue ver"),
    Default = false,
    Callback = function(value)
        if value then
...
Tabs.Client:AddSection({Translator:traduzir("Navio que cheirou acetona")})
local ShipSystem = {
    tornadoActive = false,
    currentShip = nil
}
...
local TornadoToggle = Tabs.Client:AddToggle({
    ...
    Description = Translator:traduzir("Puxa Navio Pirata para a praÃƒÂ§a"), 
    Default = false
})
...
Tabs.JJ:AddButton({
    Translator:traduzir("Pew pew, so caspa!"),
    function()
        local Anim = Instance.new("Animation")
        Anim.AnimationId = "http://www.roblox.com/asset/?id="
        local humanoid = game.Players.LocalPlayer.Character:WaitForChild("Humanoid")
...
Tabs.JJ:AddButton({
    Translator:traduzir("Vem cá vem"),
    function()
        local Anim = Instance.new("Animation")
        Anim.AnimationId = "http://www.roblox.com/asset/?id="
        local humanoid = game.Players.LocalPlayer.Character:WaitForChild("Humanoid")
...
Tabs.JJ:AddSection({Translator:traduzir("Crie um Muro de bosta")})
local selectedSize = 100 -- valor padrÃƒÂ£o

local Dropdown = Tabs.JJ:AddDropdown({
    Name = "Tamanho Da bosta",
...
Tabs.JJ:AddSection({Translator:traduzir("Crie Um Tornado de bosta")})
local selectedSize = 50

local Dropdown = Tabs.JJ:AddDropdown({
    Name = "Tamanho do Tornado de bosta",
    Description = "Escolha quantas ferramentas formarÃƒÂ£o o tornado",
...
Tabs.JJ:AddSection({Translator:traduzir("NotificaÃƒÂ§ÃƒÂµes Mapa")})
...
local function monitorPlayerDeath(player)
    ...
    if player.Character then
        ...
        if humanoid then
            humanoid.Died:Connect(function()
                if NotificationSystem.deathEnabled then
                    showNotification("Jogador levo um tiro", player.Name .. " morreu", 3)
                end
            end)
        end
    end
...
Tabs.JJ:AddToggle({
    Name = Translator:traduzir("NotificaÃƒÂ§ÃƒÂ£o: Entrada/SaÃƒÂ­da"),
...
Tabs.JJ:AddToggle({
    Name = Translator:traduzir("NotificaÃƒÂ§ÃƒÂ£o De Morte"),
...
Tabs.JJ:AddToggle({
    ...
    Description = Translator:traduzir("Torna o personagem invisÃƒÂ­vel nÃƒÂ£o volta ao normal"),
    Default = false,
    Callback = function(value)
        if value then
            local args = {
...
-- ================== SEÃƒâ€¡ÃƒÆ’O: SISTEMA DE TELEPORTE ==================
...
local playerDropdown = Tabs.JJ:AddDropdown({
    ...
    Description = Translator:traduzir("Selecione um jogador para usar as funÃƒÂ§ÃƒÂµes"),
...
Tabs.JJ:AddToggle({
    Name = Translator:traduzir("Teleporte AutomÃƒÂ¡tico"),
...
Tabs.JJ:AddToggle({
    ...
    Description = Translator:traduzir("Acompanha o jogador com a cÃƒÂ¢mera"),
    Default = false,
    Callback = function(value)
        if value then
...
local nomesRP = {
    ...
    "XALALALAL", "ANALFABETO", "CrianÃƒÂ§a", "Web namoro", "Veia", "Phantom Client", 
    "KITA!!!!", "MÃƒÆ’EEEE", "PAIII", "BANCOPAN", "Calada", "Tenho Dona: Lolyta", "Uso Hack"
}

local parteMapeada = {
    [Translator:traduzir("CabeÃƒÂ§a")] = "Head",
...
local playerDropdown = Tabs.TT:AddDropdown({
    ...
    Description = Translator:traduzir("Selecione quem o pirralho irÃƒÂ¡ seguir"),
    Options = {},
    Default = "Boneco",
...
Tabs.TT:AddDropdown({
    ...
    Description = "Onde o pirralho ficarÃƒÂ¡ preso no corpo",
    Options = {Translator:traduzir("CabeÃƒÂ§a"), Translator:traduzir("Peito Superior"), Translator:traduzir("Barriga Inferior"), Translator:traduzir("Frente do Corpo"), Translator:traduzir("Frente"), Translator:traduzir("Traseiro")},
...
Tabs.TT:AddButton({
    ...
    Callback = function()
        ...
        if not targetChar then 
            warn("Jogador alvo nÃƒÂ£o encontrado no workspace.") 
            return 
        end
        ...
        if pirralho and parteAlvo then
            ...
            warn("A parte '" .. parteSelecionada .. "' nÃƒÂ£o existe no jogador alvo.")
        end
    end
})
...
local Button = Tabs.TT:AddButton({
    ...
    Description = Translator:traduzir("Crie a usina Nuclear na praÃƒÂ§a"),
    Callback = function()
...
"hi\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\rShinra Tensei!"
        ...
        if sound.Parent then
            ...
            warn("HumanoidRootPart nÃƒÂ£o encontrado")
        end
        ...
        local success, errorMessage = pcall(function()
            ...
            if not currentCharacter or not currentCharacter:FindFirstChild("HumanoidRootPart") then
                warn("Personagem nÃƒÂ£o encontrado durante a explosÃƒÂ£o!")
                return
            end
...
local ButtonStart = Tabs.TT:AddButton({Translator:traduzir("Shinra Tensei by hiro"), function()
...
Tabs.TT:AddButton({
    Name = "ExpansÃƒÂ£o de DomÃƒÂ­nio By Hiro",
    Callback = function()
        if expansionActive then
            warn("ExpansÃƒÂ£o jÃƒÂ¡ estÃƒÂ¡ ativa!")
            return
        end
        ...
        if TextChatService.ChatVersion == Enum.ChatVersion.TextChatService then 
            TextChatService.TextChannels.RBXGeneral:SendAsync(
                "hi\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\rExpansÃƒÂ£o de DomÃƒÂ­nio"
            )
        end
...
Tabs.TT:AddButton({
    Name = "Haki Do Rei CrÃƒÂ©ditos: Drip Client",
    Callback = function()
        _G.HakiAtived = true
        _G.StopHaki = false
...
Tabs.TT:AddButton({
    ...
    Description = Translator:traduzir("Stop/Shinra Tensei/Haki do Rei/ExpansÃƒÂ£o/Mangekyou"),
    Callback = function()
        ----------------------------
        -- Shinra Tensei
        ----------------------------
        fireLoopActiveServer = false
        ignoreTarget = nil
        -- remove explosÃƒÂµes/esferas criadas
        ...
        -- remove partÃƒÂ­culas aplicadas no corpo
        ...
        -- remove partÃƒÂ­culas (chamas pretas) dos players
        ...
        warn("Ã¢Å“â€¦ Todas as habilidades e efeitos foram parados e removidos!")
    end
})
...
local ToggleBlackHole = Tabs.TT:AddToggle({
    ...
    Description = "NÃƒÂºcleo blackHole Hacking!!!",
    Default = false
})
...
local function BringWithCouch()
    ...
    if not couch then
        warn("Erro: SofÃƒÂ¡ nÃƒÂ£o encontrado no Backpack")
        return
    end

    couch.Name = "Phantom.Couch"
...
local function KillWithCouch()
    ...
    if seat1 and seat2 and handle then
        ...
        warn("Erro: Componentes do sofÃƒÂ¡ nÃƒÂ£o encontrados")
        return
    end
    couch.Parent = LocalPlayer.Character
...
local function jailcouch()
    ...
    Tabs.TT:AddDropdown({
        Name = Translator:traduzir("Escolhar um MÃƒÂ©todo <KILL>"),
...
Tabs.TT:AddButton({
        ...
        Callback = function()
            ...
            elseif methodKill == "Couch de longe" then
                ...
                if not humanoidRootPart then
                    warn("Erro: HumanoidRootPart do jogador local nÃƒÂ£o encontrado")
                    return
                end
    ...
    Tabs.TT:AddButton({
        ...
        Callback = function()
            ...
            if methodKill == "Couch" then
                KillPlayerCouch()
            elseif methodKill == "Couch Sem ir atÃƒÂ© o alvo [BETA]" then
                KillWithCouch()
            else
                            ...
                            if targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
                                ...
                                if targetHumanoid and targetHumanoid.Sit then
                                    if character.Humanoid then
                                        ...
                                        print("Jogador sentou, levando ÃƒÂ´nibus para o void!")
                                        task.wait(0.2)
                                        ...
                                        print("Player voltou para a posiÃƒÂ§ÃƒÂ£o inicial.")
                                    end
                                    break
                                else
                                    local targetRoot = targetPlayer.Character.HumanoidRootPart
...
Tabs.TT:AddButton({
    ...
    Callback = function()
        if not selectedPlayerName or not game.Players:FindFirstChild(selectedPlayerName) then
            warn("Nenhum jogador selecionado ou nÃƒÂ£o existe")
            return
        end
        ...
        if not Humanoid or not RootPart then
            warn("Humanoid ou RootPart invÃƒÂ¡lido")
            return
        end
        ...
        if not Seat then
            warn("Assento nÃƒÂ£o encontrado")
            return
        end
        repeat 
            task.wait(0.1)
        ...
        if not TargetRP or not TargetH then
            warn("Humanoid ou RootPart do alvo nÃƒÂ£o encontrado")
            return
        end
...
do
Tabs.TT:AddSection({Translator:traduzir("FunÃƒÂ§ÃƒÂµes Mata <Todos>")})
Tabs.TT:AddButton({
    Name = Translator:traduzir("Mata Geral De Ãƒâ€nibus"),
    Callback = function()
        local Players = game:GetService("Players")
local Workspace = game:GetService("Workspace")
local RunService = game:GetService("RunService")
...
--[[    local function getThumbnail(player)
    ...
    local function getSoccerBall()
        do
            ...
            if not tool then return warn("Bola nÃƒÆ’Ã‚Â£o encontrada.") end
    ...
    Tabs.TT:AddButton({
        ...
        Callback = function()
            Window:Dialog({
                ...
                Text = Translator:traduzir("Equipe a bola e use o clique para arremessar jogadores. Ferramenta com cÃƒÂ¢mera automÃƒÂ¡tica."),
                Options = {
                    {"Link Discord", function()
                        copyDiscordLink()
                    end},
...
Tabs.TT:AddButton({
    Translator:traduzir("Guitarra elÃƒÂ©trica + ViolÃƒÂ£o - Acustic"),
    function()
...
local Button = Tabs.TT:AddButton({Translator:traduzir("Pega SofÃƒÂ¡"), function()
...
Tabs.TT:AddToggle({
    Name = Translator:traduzir("Voar Sobre a cabeÃƒÂ§a"),
    Default = false,
...
local lagsv = Tabs.LG:AddDropdown({
    ...
    Description = Translator:traduzir("Escolha qual jogador ficarÃƒÂ¡ para trÃƒÂ¡s"),
    Options = PlayerList,
    Default = "Players",
    Callback = function(Value)
...
local dropdon = Tabs.LG:AddDropdown({
    Title = "Selecionar um mÃƒÂ©todo",
    Description = Translator:traduzir("Selecione um mÃƒÂ©todo para Laga a pessoa"),
    Options = {"Phone", "Laptop", "Phone and Laptop", "Basketball", "Ghost Hunter", "Book", "Stretcher", "FireX", "Bomb"},
    Default = "Method",
    Callback = function(Value)
        if TargetLagActive then
...
Tabs.LG:AddToggle({
    Title = "Lag Player",
    Description = Translator:traduzir("Apos seleciona use essa funÃƒÂ§ÃƒÂ£o"),
    Default = false,
    Callback = function(Value)
        if Value then
...
local IgnoreDropdown = Tabs.LG:AddDropdown({
    Name = Translator:traduzir("Quem nÃƒÂ£o serÃƒÂ¡ bugado?"),
    Description = Translator:traduzir("Escolhar um jogador pra nÃƒÂ£o toma bugue server"),
    Options = {},
    Default = "Players",
...
Tabs.LG:AddToggle({
    ...
    Description = Translator:traduzir("Buga geral menos vocÃƒÂª, ÃƒÂ© sÃƒÂ³ assistir"),
    Default = false,
...
local Dropdown = Tabs.LG:AddDropdown({
    Name = Translator:traduzir("Selecione MÃƒÂ©todo de lags"),
    Description = Translator:traduzir("Novos mÃƒÂ©todos de lags dripsolutions"),
    Options = options,
    Default = "Common",
...
local Toggle = Tabs.LG:AddToggle({
    Name = Translator:traduzir("ComeÃƒÂ§a Lags"), 
...
Tabs.AD:AddSection({Translator:traduzir("MÃƒÂºsicas Especiais - SERVIDOR")})

local SpecialMusics = {
    {name = "Retro Phonk", id = 73859601978760},
    {name = "Phonk", id = 119202700760169},
    {name = "Jumpstyle", id = 700891754},
    {name = "Brasileiro", id = 111621504412077},
    {name = "From the start - good kids", id = 94040688375495},
    -- Novas mÃƒÂºsicas adicionadas
    ...
    {name = "tomagi mo tÃƒÂ£o", id = 111668097052966},
    ...
    {name = "Essa ÃƒÂ© Antiga", id = 90999978343771},
    {name = "Essa Aqui ÃƒÂ© Braba", id = 120833520065453},
    {name = "Help Me Phonk", id = 72458292563094},
    {name = "Envolve A Tropa", id = 88094479399489},
    {name = "No Pick Mulher", id = 92893359226454},
    {name = "MÃƒÂ­dia", id = 105832154444494},
    ...
    {name = "Phonk InglÃƒÂªs", id = 14145624031},
    ...
    {name = "NÃƒÂ£o Estou louco", id = 74366765967475},
    {name = "Manda Meu Passinho", id = 77741294709660},
    {name = "CenÃƒÂ¡rio Selvagem", id = 117562011395992},
...
-- Ordenar as mÃƒÂºsicas por nome para ficar mais organizado
table.sort(SpecialMusics, function(a, b) 
    return a.name:lower() < b.name:lower()
end)

local selectedSpecialMusic = nil
local specialMusicLoop = false
local specialMusicThread = nil

-- Extrair nomes para a dropdown
local specialMusicNames = {}
for _, music in ipairs(SpecialMusics) do
    table.insert(specialMusicNames, music.name)
end

-- Encontrar ID pelo nome
local function findSpecialMusicByName(name)
    for _, music in ipairs(SpecialMusics) do
...
-- FunÃƒÂ§ÃƒÂ£o para tocar ÃƒÂ¡udio no servidor (todos ouvem)
local function playAudioOnServer(audioId)
    local RE = ReplicatedStorage:FindFirstChild("RE")
    if RE then
        local soundEvent = RE:FindFirstChild("1Gu1nSound1s")
        if soundEvent then
            pcall(function()
                soundEvent:FireServer(workspace, audioId, 1)
                print("Ã°Å¸Å½Âµ ÃƒÂudio enviado para o servidor: " .. audioId)
            end)
        else
            warn("Ã¢ÂÅ’ RemoteEvent 1Gu1nSound1s nÃƒÂ£o encontrado!")
        end
    else
        warn("Ã¢ÂÅ’ Pasta RE nÃƒÂ£o encontrada no ReplicatedStorage!")
    end
end

-- FunÃƒÂ§ÃƒÂ£o para tocar ÃƒÂ¡udio localmente
local function playAudioLocal(audioId)
    local character = LocalPlayer.Character
    if character and character:FindFirstChild("HumanoidRootPart") then
        local sound = Instance.new("Sound")
        ...
        print("Ã°Å¸â€Å  ÃƒÂudio tocando localmente: " .. audioId)
    else
        warn("Ã¢ÂÅ’ Character ou HumanoidRootPart nÃƒÂ£o encontrado!")
    end
end

Tabs.AD:AddDropdown({
    Name = Translator:traduzir("Selecionar MÃƒÂºsica Especial"),
    Description = Translator:traduzir("Ã°Å¸Å½Âµ " .. #specialMusicNames .. " mÃƒÂºsicas disponÃƒÂ­veis - TODOS OUVEM"),
    Options = specialMusicNames,
    Default = "Escolha uma mÃƒÂºsica",
    Callback = function(value)
        local musicId = findSpecialMusicByName(value)
        if musicId then
            selectedSpecialMusic = musicId
            redzlib:Notify({
                Title = "Ã°Å¸Å½Âµ MÃƒÂºsica Selecionada",
                Description = value .. " selecionada! (ID: " .. musicId .. ")",
                Type = "Success",
                Duration = 3
            })
        end
...
Tabs.AD:AddButton({
    Name = Translator:traduzir("Ã¢â€“Â¶Ã¯Â¸Â Tocar MÃƒÂºsica (SERVIDOR)"),
    Description = Translator:traduzir("Tocar para TODOS os jogadores ouvirem"),
    Callback = function()
        if selectedSpecialMusic then
            -- Tocar no servidor (todos ouvem)
            playAudioOnServer(selectedSpecialMusic)
            
            -- Tocar localmente tambÃƒÂ©m
            playAudioLocal(selectedSpecialMusic)
            
            redzlib:Notify({
                Title = "Ã°Å¸Å½Âµ Tocando MÃƒÂºsica",
                Description = "MÃƒÂºsica tocando para todo o servidor!",
                Type = "Success",
                Duration = 3
            })
        else
            redzlib:Notify({
                Title = "Ã¢ÂÅ’ Erro",
                Description = "Selecione uma mÃƒÂºsica primeiro!",
                Type = "Error",
                Duration = 3
            })
        end
...
Tabs.AD:AddButton({
    Name = Translator:traduzir("Ã°Å¸â€â€¡ Parar MÃƒÂºsica"),
    Description = Translator:traduzir("Parar mÃƒÂºsica atual"),
    Callback = function()
        -- Parar loop se estiver ativo
        specialMusicLoop = false
        if specialMusicThread then
            task.cancel(specialMusicThread)
            specialMusicThread = nil
        end
        ...
        redzlib:Notify({
            Title = "Ã¢ÂÂ¹Ã¯Â¸Â MÃƒÂºsica Parada",
            Description = "MÃƒÂºsica interrompida!",
            Type = "Warning",
            Duration = 3
        })
    end
...
Tabs.AD:AddToggle({
    Name = Translator:traduzir("Ã°Å¸â€â€ž Loop MÃƒÂºsica Especial"),
    Description = Translator:traduzir("Repetir continuamente para o servidor"),
    Default = false,
    Callback = function(value)
        specialMusicLoop = value
        
        if value then
            if selectedSpecialMusic then
                specialMusicThread = task.spawn(function()
                    while specialMusicLoop do
                        -- Tocar no servidor
                        playAudioOnServer(selectedSpecialMusic)
                        ...
                        -- Esperar antes de repetir (ajuste conforme necessÃƒÂ¡rio)
                        task.wait(15) -- 15 segundos entre repetiÃƒÂ§ÃƒÂµes
                    end
                end)
                
                redzlib:Notify({
                    Title = "Ã°Å¸â€Â Loop Ativado",
                    Description = "MÃƒÂºsica em loop para o servidor!",
                    Type = "Success",
                    Duration = 3
                })
            else
            ...
            redzlib:Notify({
                Title = "Ã¢ÂÂ¹Ã¯Â¸Â Loop Desativado",
                Description = "Loop de mÃƒÂºsica interrompido!",
                Type = "Warning",
                Duration = 3
            })
        end
...
-- Adicionar tambÃƒÂ©m para tocar no rÃƒÂ¡dio do carro/casa
Tabs.AD:AddButton({
    Name = Translator:traduzir("Ã°Å¸Å½Â¶ Tocar no RÃƒÂ¡dio (GamePass)"),
    Description = Translator:traduzir("Tocar mÃƒÂºsica no rÃƒÂ¡dio do carro/casa"),
    Callback = function()
        if selectedSpecialMusic then
            local args = {
                [1] = "ToolMusicText",
                [2] = tostring(selectedSpecialMusic)
            }
            local remote = game:GetService("ReplicatedStorage"):WaitForChild("RE"):WaitForChild("PlayerToolEvent")
            if remote then
                remote:FireServer(unpack(args))
                redzlib:Notify({
                    Title = "Ã°Å¸â€œÂ» RÃƒÂ¡dio",
                    Description = "MÃƒÂºsica tocando no rÃƒÂ¡dio!",
                    Type = "Success",
                    Duration = 3
                })
            else
                redzlib:Notify({
                    Title = "Ã¢ÂÅ’ Erro RÃƒÂ¡dio",
                    Description = "RemoteEvent do rÃƒÂ¡dio nÃƒÂ£o encontrado!",
                    Type = "Error",
                    Duration = 3
                })
            end
...
local TROLL_AUDIOS = {
    ...
    {name = "AÃƒÂ­ Meu Cuzinhorrr", id = 130714479795369},
    {name = "ZÃƒÂ© da MangÃƒÂ¡", id = 106624090319571},
    ...
    {name = "TÃƒÂ¡ Querendo Ram", id = 94395705857835},
    {name = "Hoje Vou Ser Tua Mulher", id = 90844637105538},
    {name = "Vc TÃƒÂ¡ Fudido", id = 120214772725166},
    {name = "RojÃƒÂ£o Chiiiin", id = 6549021381},
    {name = "Posso Te Ligar Agora", id = 103211341252816},
    {name = "CadÃƒÂª o ZÃƒÂ© Gotinha?", id = 86012585992725},
    {name = "A Pia Cheia de LouÃƒÂ§a", id = 98076927129047},
    {name = "Olha Mensagemmmmm", id = 121668429878811},
    {name = "Olha um PÃƒÂ© de Siriguela", id = 112804043442210},
    ...
    {name = "Nobru ApelÃƒÂ£o", id = 77627828376120},
...
local LOUD_AUDIOS = {
    ...
    {name = "Quero CafÃƒÆ’Ã‚Â©..", id = 113702985238621},    
    ...
    {name = "Mega ÃƒÂudio", id = 6855150757},
    ...
    {name = "PotÃƒÂªncia", id = 7520729342},
    {name = "Cinema", id = 91412024101709},
    {name = "Estora TÃƒÂ­mpano", id = 268116333},
...
function AudioManager.new()
    ...
    local function fireServerAudio(audioId)
        ...
        if soundEvent then
            ...
            if gunSoundEvent then
                ...
                if not success then
                    warn("Erro ao reproduzir ÃƒÂ¡udio no servidor: " .. tostring(err))
                end
            else
                warn("Evento de som nÃƒÂ£o encontrado")
            end
        else
            warn("ReplicatedStorage.RE nÃƒÂ£o encontrado")
        end
    end
    
    function self:playAudio(audioId, playOnServer)
        if not audioId then
            warn("ID de ÃƒÂ¡udio nÃƒÂ£o especificado")
            return
        end
    ...
    function self:startTrollLoop()
        if not self.selectedTrollAudio then
            warn("Nenhum ÃƒÂ¡udio de troll selecionado")
            return
        end
    ...
    function self:startLoudLoop()
        if not self.selectedLoudAudio then
            warn("Nenhum ÃƒÂ¡udio estourado selecionado")
            return
        end
...
Tabs.AD:AddSection({Translator:traduzir("ÃƒÂudios de Trollagem")})
Tabs.AD:AddDropdown({
    Name = Translator:traduzir("Escolher ÃƒÂudio de Troll"),
    Description = Translator:traduzir("Selecione um ÃƒÂ¡udio para trollar"),
    Options = trollAudioNames,
    Default = "Selecione um ÃƒÂ¡udio",
    Callback = function(value)
...
local audioId
Tabs.AD:AddTextBox({
    Name = Translator:traduzir("Insira o ID do ÃƒÂudio ou Musica"),
    Description = Translator:traduzir("Digite o ID do ÃƒÂ¡udio"),
    PlaceholderText = "NÃƒÂºmeros",
    Callback = function(value)
...
Tabs.AD:AddButton({
    ...
    Description = Translator:traduzir("Reproduzir o ÃƒÂ¡udio selecionado uma vez"),
    Callback = function()
        if audioManager.selectedTrollAudio then
            audioManager:playAudio(audioManager.selectedTrollAudio, true)
        else
            warn("Selecione um ÃƒÂ¡udio primeiro!")
        end
    end
})
...
Tabs.AD:AddToggle({
    ...
    Description = Translator:traduzir("Repetir continuamente o ÃƒÂ¡udio selecionado"),
    Default = false,
    Callback = function(value)
        if value then
...
Tabs.AD:AddSection({Translator:traduzir("ÃƒÂudios Estourados")})

Tabs.AD:AddDropdown({
    Name = Translator:traduzir("Escolher ÃƒÂudio Estourado"),
    Description = Translator:traduzir("Selecione um ÃƒÂ¡udio estourado"),
    Options = loudAudioNames,
    Default = Translator:traduzir("Selecione um ÃƒÂ¡udio"),
    Callback = function(value)
...
Tabs.AD:AddButton({
    ...
    Description = Translator:traduzir("Reproduzir o ÃƒÂ¡udio estourado uma vez"),
    Callback = function()
...
Tabs.AD:AddToggle({
    ...
    Description = Translator:traduzir("Repetir continuamente o ÃƒÂ¡udio estourado"),
    Default = false,
    Callback = function(value)
        if value then
...
Tabs.AD:AddSection({Translator:traduzir("ConfiguraÃƒÂ§ÃƒÂµes Especiais")})

Tabs.AD:AddToggle({
    Name = Translator:traduzir("ÃƒÂudio Estourado RÃƒÂ¡pido"),
    Description = Translator:traduzir("Loop ultra rÃƒÂ¡pido do ÃƒÂ¡udio padrÃƒÂ£o (cuidado!)"),
    Default = false,
    Callback = function(value)
...
local Toggle = Tabs.AD:AddToggle({
    ...
    Description = "Bloqueia apenas ÃƒÂ¡udios trolls especÃƒÂ­ficos", 
    Default = false
})
...
Tabs.AD:AddSection({Translator:traduzir("MÃƒÂºsica no RÃƒÂ¡dio <Game Pass>")})
...
local Dropdown = Tabs.AD:AddDropdown({
	Name = Translator:traduzir("Selecionar MÃƒÂºsica"),
	Description = Translator:traduzir("Toca no rÃƒÂ¡dio via ToolMusicText"),
	Options = DripRadio,
	Default = "Escolha uma mÃƒÂºsica",
	Callback = function(Value)
...
Tabs.AD:AddButton({Translator:traduzir("Tocar MÃƒÂºsica no RÃƒÂ¡dio <FM>"), function()
...
Tabs.AD:AddButton({Translator:traduzir("Pegar RÃƒÂ¡dio <FM>"), function()
	local args = {
    [1] = "PickingTools";
    [2] = "Boombox";
}
...
Tabs.AD:AddButton({Translator:traduzir("Parar MÃƒÂºsica do RÃƒÂ¡dio <FM>"), function()
	local args = {
		[1] = "ToolMusicStop",
		[2] = "" 
	}
...
-----------------///////TABS - AVT///////-----------------
do
Tabs.AVT:AddSection({Translator:traduzir("AnimaÃƒÂ§ÃƒÂ£o & Reset")})
...
Tabs.AVT:AddButton({Translator:traduzir("AnimaÃƒÂ§ÃƒÂ£o home ou muie"), function()
    local args = {
    [1] = 4418326547
}
...
Tabs.AVT:AddButton({Translator:traduzir("CabeÃƒÂ§a InvisÃƒÂ­vel"), function()
    local args = {
    [1] = 134082579
}
...
Tabs.AVT:AddButton({
    Name = "1x1x1x",
    Description = "Equipa conjunto 1x1x1x",
    Callback = function()
        local ids = {76383563810685, 136993365938209, 102827435656799, 15188487665}
        
        for _, id in ipairs(ids) do
            local args = { [1] = id }
            game:GetService("ReplicatedStorage").Remotes.Wear:InvokeServer(unpack(args))
            task.wait(0.3) -- Pequena pausa entre cada item
        end
        
        redzlib:Notify({
            Title = "1x1x1x Equipado",
            Description = "Conjunto especial equipado com sucesso!",
            Type = "Success",
            Duration = 4
        })
    end
...
Tabs.AVT:AddSlider({
    Name = Translator:traduzir("VerificaÃƒÂ§ÃƒÂ£o De Parts <Update>"),
    Min = 5,
    Max = 15,
    Increase = 1,
    Default = 25,
...
if not Remotes then
    warn("Pasta Remotes nÃƒÂ£o encontrada.")
    return
end
...
if TPlayer and TPlayer.Character then
    ...
    if LHumanoid and THumanoid then
        ...
        if remoteBody then
            ...
            warn("RemoteFunction 'ChangeCharacterBody' nÃƒÂ£o encontrado.")
        end
...
Tabs.AVT:AddSection({Translator:traduzir("Super HerÃƒÂ³is <Packs>")})
...
local roupasCompletas = {
    ["ADM <HiroDev>"] = {77933716022362,73722333789629,91122615317042},
}
...
Tabs.AVT:AddButton({Translator:traduzir("ElegÃƒÂ¢ncia Rosa-Pastel"), function()
    aplicarPackCompleto(
...
Tabs.AVT:AddButton({Translator:traduzir("InformaÃƒÂ§ÃƒÂµes De Uso"), function()
Window:Dialog({
    Title = "Fun",
    Text = Translator:traduzir("Ã¢â‚¬Â¢As funÃƒÂ§ÃƒÂµes acima jÃƒÂ¡ aplicam as roupas completas, portanto, basta apenas modificar o corpo ou a cor."),
    Options = {
        {"Fechar", function()
...
Tabs.AVT:AddDropdown({
	...
	Description = Translator:traduzir("Roupas 3D Varias opÃƒÂ§ÃƒÂµes Tops!"),
	Options = skinNames,
	Default = "Roupas",
	Callback = function(Value)
...
do
Tabs.AVT:AddSection({Translator:traduzir("Especiais Efeitos <AcessÃƒÂ³rios>")})

local acessoriosEspeciais = {
    ["Fada do Outono"] = 128217885,
    ["Fada da Primavera"] = 150381051,
    ["Fada do Dia de SÃƒÂ£o Petersburgo"] = 226189871,
...
Tabs.AVT:AddDropdown({
    ...
    Description = Translator:traduzir("Selecione um acessÃƒÂ³rio para ativar"),
    Options = nomes,
    Default = "Efeitos",
...
Tabs.AVT:AddButton({
    Translator:traduzir("Vestir AcessÃƒÂ³rio"),
    function()
...
-----------------///////TABS - HouseTab///////-----------------
do
Tabs.HouseTab:AddSection({Translator:traduzir("MÃƒÂºsica na Casa (Game Pass)")})
local selectedMusic = ""
local MusicsList = {
	{ Name = "fani", Id =  10561310081763},
	{ Name = "Noite ElÃƒÂ©trica", Id =  112651304685397},
...
Tabs.HouseTab:AddTextBox({
	Name = Translator:traduzir("Insira o ID da MÃƒÂºsica"),
	Description = Translator:traduzir("VocÃƒÂª pode colar o ID direto aqui"),
...
Tabs.HouseTab:AddButton({Translator:traduzir("Tocar MÃƒÂºsica"), function()
...
Tabs.HouseTab:AddButton({Translator:traduzir("Parar MÃƒÂºsica"), function()
	local args = {
...
Tabs.HouseTab:AddSection({Translator:traduzir("FunÃƒÂ§ÃƒÂµes AvanÃƒÂ§adas <Casas>")})
...
local function changeColor()
    ...
    if not remoteEvent then
        warn("RemoteEvent '1Player1sHous1e' nÃƒÂ£o encontrado.")
        return
    end
...
Tabs.HouseTab:AddSection({Translator:traduzir("FunÃƒÂ§ÃƒÂµes AvanÃƒÂ§ada Da Casa")})
...
Tabs.Carros:AddTextBox({
	...
	Description = Translator:traduzir("Velocidade do carro preencher o espaÃƒÂ§o"),
...
local MusicsList = {
    ...
    { Name = "MÃƒÂºsicas de Piano acÃƒÂºstico", Id = 1838457617 },
...
Tabs.Carros:AddButton({Translator:traduzir("Tocar MÃƒÂºsica"), function()
...
Tabs.Carros:AddButton({Translator:traduzir("Para MÃƒÂºsica"), function()
	local args = {
		[1] = "StopMusic",
		[2] = 73423225643184
	}
...
Tabs.Carros:AddSection({Translator:traduzir("Carro FunÃƒÂ§ÃƒÂµes")})
local CarrosTT = Tabs.Carros:AddDropdown({
    Name = Translator:traduzir("Selecione um carro"),
    Description = Translator:traduzir("Selecionar Um Carro para as funÃƒÂ§ÃƒÂµes"),
...
Tabs.Carros:AddSlider({
    Name = Translator:traduzir("VerificaÃƒÂ§ÃƒÂ£o de Carros"),
    Min = 1,
    Max = 25,
    Increase = 1,
    Default = 25,
...
Tabs.Carros:AddToggle({
	...
	Description = Translator:traduzir("Teleporta temporariamente o carro atÃƒÂ© vocÃƒÂª"),
	Default = false,
	Callback = function(Value)
            ...
            if not vehicle:FindFirstChild("ESP") and vehicle:IsA("Model") and vehicle.PrimaryPart then
                ...
                local connection = game:GetService("RunService").RenderStepped:Connect(function()
                    if ESPEnabled and vehicle and vehicle.PrimaryPart then
                        ...
                        textLabel.Text = vehicle.Name .. " | DistÃƒÂ¢ncia: " .. math.floor(distance) .. " studs"
                    end
                end)
...
Tabs.Carros:AddSection({Translator:traduzir("Drip AvanÃƒÂ§ado")})
...
Tabs.Carros:AddSection({Translator:traduzir("SeÃƒÂ§ÃƒÂ£o Kill Car")})
local selectedPlayer = nil
...
local Players5 = Tabs.Carros:AddDropdown({
    ...
    Description = Translator:traduzir("Selecionar um jogador pra funcionar as funÃƒÂ§ÃƒÂµes"),
    Options = {},
    Default = "Players",
    Callback = function(value)
...
local Button = Tabs.Carros:AddButton({Translator:traduzir("InÃƒÂ­cia Fling"), function()
    if selectedPlayer then
        flingLoop = true
        FlingPlayer()
    end
...
--[[Tabs.PR:AddSection({"MudanÃƒÂ§as de Tempos"})
...
local teleportButtons = {
    {Nome = "AgÃƒÂªncia", Ponto = CFrame.new(179, 4, -464)},
    ...
    {Nome = "ÃƒÂrea do Crime", Ponto = CFrame.new(-119, -28, 235)},
    ...
    {Nome = "HotÃƒÂ©is", Ponto = CFrame.new(182, 4, 150)},
    {Nome = "Montanha", Ponto = CFrame.new(-670, 251, 765)},
    {Nome = "Portal da AgÃƒÂªncia", Ponto = CFrame.new(672, 4, -296)},
    {Nome = "Esconderijo", Ponto = CFrame.new(505, -75, 143)},
    {Nome = "Escola", Ponto = CFrame.new(-312, 4, 211)},
    {Nome = "CafÃƒÂ©", Ponto = CFrame.new(161, 8, 52)},
    {Nome = "Cafeteria", Ponto = CFrame.new(-97.12, 4.65, 254.99)},
    {Nome = "InÃƒÂ­cio", Ponto = CFrame.new(-26, 4, -23)},
    {Nome = "Banco", Ponto = CFrame.new(2.28, 4.65, 254.58)},
    {Nome = "Roupas", Ponto = CFrame.new(-46.15, 4.65, 253.20)},
    {Nome = "RefÃƒÂºgio", Ponto = CFrame.new(-88.48, 22.05, 262.34)},
    {Nome = "ClÃƒÂ­nica DentÃƒÂ¡ria", Ponto = CFrame.new(-53.58, 22.15, 265.61)},
}
...
Tabs.Tp:AddButton({
    ...
    Callback = function()
        ...
        if not plr.Character or not plr.Character:FindFirstChild("HumanoidRootPart") then
            warn("Personagem nÃƒÂ£o encontrado!")
            return
        end
...
-- Seleciona o primeiro local por padrÃƒÂ£o
...
-----------------///////TABS - CFG///////----------------
Tabs.CFG:AddSection({Translator:traduzir("Sistema De VariaÃƒÂ§ÃƒÂµes")})
local antiSitConnections = {}
local isAntiSitActive = false

Tabs.CFG:AddToggle({
    Name = Translator:traduzir("Anti Sit"),
    Description = Translator:traduzir("NÃƒÂ£o Sentar"),
    Default = false,
    Callback = function(Value)
        local player = game.Players.LocalPlayer
        local runService = game:GetService("RunService")
...
Tabs.CFG:AddToggle({Name = Translator:traduzir("This Does Nothing"), Description = Translator:traduzir("Does Nothing"), Default = true})
...
local function stopGuiWatcher(guiName)
    ...
    local target = workspace:FindFirstChild(guiName == "Hiro")
...
---------------------------------------------------------------------------------------------------------------------------------
                                                   -- === Tab 18: ProteÃƒÂ§ÃƒÂ£o 2 === --
---------------------------------------------------------------------------------------------------------------------------------

local Tab18 = Window:MakeTab({"Protection 2", "rbxassetid://11322093465"})
...
Tab18:AddToggle({
    Name = "Anti Fling HelicÃƒÂ³pteros",
    Description = "",
    Default = false,
